...
if withXSUncertainty
    ...
    XSDIR = '/home/akgray/neutronics/IntervalParticleTransportMatlab/tendlHDF5/Fe056';

    dir = [XSDIR,'/Fe056_0000.h5'];
    ...
    for i =1:numFe
        ...
        scatFe(i,:) = h5read([XSDIR,files],'/elastic');
        abspFe(i,:) = h5read([XSDIR,files],'/absorption');
        totalFe(i,:) = h5read([XSDIR,files],'/total');
    end
    ...
    filename = '/home/akgray/neutronics/IntervalParticleTransportMatlab/tendlHDF5/Fe056/Fe056_0000.h5';
...
parfor  j =1:numBatch
    ...
    for i = 1:numParticles
        ...
        particleEnergy = [13.2e6,14.2e6].*normrnd(0,14.2e6*0.05);
...
if totalMC
    ...
    numSimulations = 10;
    numBatch = 10;
    tendl                       = unidrnd( numFe, [numSimulations*numSources, 1]);
    current = [0,0];
    particleEnergy = [13.2e6,14.2e6];
    positionUncertainty(1) = 0;
    current(1,2) = positionUncertainty(1) + current(1,1);
    tmcTallyMean = zeros(numel(energyTally),numSimulations*numSources);
    tmcTallyVar = zeros(numel(energyTally),numSimulations*numSources);
    for ll = 1:numSources
        sourceMean = (particleEnergy(2)-particleEnergy(1)).*rand() + particleEnergy(1);
        for k = 1:numSimulations
            ...
            parfor j = 1: numBatch
                 ...
                 totes = repmat(totalFe(tendl((ll-1)*10+k), :)',[1,2]); 
                 scats = repmat(scatFe(tendl((ll-1)*10+k), :)',[1,2]);
                 abps  = repmat(abspFe(tendl((ll-1)*10+k), :)',[1,2]);
                ...
                for i = 1:numParticles
                    Alive = 1; 
                    pointParticleEnergy         = repmat(normrnd(sourceMean,14.2e6*0.05),[1,2]); %repmat((particleEnergy(1,1) + (particleEnergy(1,2) - particleEnergy(1,1)) .* rand(1,1))', [1,2]);
            ...
            for l = 1:numel(energyTally)
                tmcTallyMean(l,(ll-1)*10+k) = mean(tmcTallySingle(:,l));
                tmcTallyVar(l,(ll-1)*10+k) = var(tmcTallySingle(:,l));
            end
        end
    end
    toc
...
save('AnderIntervalTransportSaveUncertainSource');
% 
...
function [particleEnergyOut, particlePositionOut, alive] = transport(particleEnergyIn, particlePositionIn, total, scat, absp, energy, randSeq)
    ...
    probAbsp = [1/(1+(scatxs(2)/abspxs(1))),1/(1+(scatxs(1)/abspxs(2)))];
